...
\						sodio                       & $2$ & $8$ &      $1$       \\ 
        ...
        \subsubsection{Classificazione dei documenti}\label{sec:classDoc}
			...
			\begin{itemize}
				\item \texttt{\textbackslash \hicode{itemVE}} - per i verbali esterni;
				\item \texttt{\textbackslash \hicode{itemVI}}  - per quelli interni.
			\end{itemize}
        ...
        \subsubsection{Procedure}
            ...
            Viene fornita nel \glo{Repository}{repository} Documenti, la cartella \hicode{Modello nuovo documento} contenente la struttura di base che ogni documento deve avere. Al suo interno sono presenti: 
            \begin{itemize}
            	\item \textbf{il file principale del documento:} \hicode{ModelloNuovoDocumento.tex};
            	\item \textbf{la cartella che contiene i file dei capitoli:} \hicode{sections};
            	\item \textbf{la cartella che contiene le immagini:} \hicode{img}.
            \end{itemize}  
...
%            Per un corretto funzionamento del glossario e del sistema di linking dei file \LaTeX{} bisogna necessariamente, ma solo la prima volta che si crea il documento, prima compilare il glossario e successivamente il documento appena creato. 
	        ...
	        \begin{enumerate}
	        	\item entrare nello spazio condiviso del gruppo su \glo{Google Drive}{Google Drive} (si veda sezione \ref{sec:GoogleDrive}) e aprire il foglio \hicode{Termini da inserire nel glossario};
	        	\item inserire il termine in fondo alla pagina.
	        \end{enumerate}
        	%
        	\paragraph{Inserimento termini a glossario nei Manuali}
        	...
        	È stato scelto di usare il pacchetto \LaTeX \hicode{\{glossaries\}} per questo tipo di glossari. Per il suo corretto funzionamento, questo comando, necessita di compilazioni separate rispetto a quelle effettuate da TeXstudio. Ulteriori informazioni riguardo a questo pacchetto si possono trovare alla pagina:
        	\begin{center}
        		\url{https://it.sharelatex.com/learn/Glossaries}
        	\end{center}
      		%
        	Per automatizzare questa procedura è stato creato uno script \hicode{./mkglo.sh} da utilizzare solo quando si inseriscono nuovi termini nei manuali. \\ \\
        	%
        	Di seguito è descritta la procedura da seguire \textbf{ogni volta} che si desidera inserire un termine a glossario: 
        	\begin{enumerate}
        		\item inserire nel testo il comando \hicode{\textbackslash gls\{NomeTermine\}};
        		\item aprire il file \LaTeX \hicode{GlossarioManUtente.tex} oppure \hicode{GlossarioManMan.tex} contenente la definizione dei termini a glossario;
        		...
        		\item (opzionale se non si hanno i permessi giusti per lo script) digitare il comando \hicode{chmod +x mkglo.sh};
        		\item digitare il comando \hicode{./ mkglo.sh NomeManuale.tex}.
        	\end{enumerate}
        	%
  ...
  \subsubsection{Versionamento}\label{sec:versionamento}
	  ...
	  \begin{itemize}
	  	...
	  	\item eseguire il comando sudo \hicode{apt-get update};
	  	\item eseguire il comando \hicode{apt-get install git}.
	  \end{itemize}
	  ...
	  \begin{itemize}
	  	...
	  	\item scaricare l'eseguibile;
	  	\item aprire il file appena scaricato e avviare l'installazione cliccando sul file \hicode{.pkg}.
	  \end{itemize}
	  ...
	  \begin{itemize}
	  	\item \hicode{{git} config --global user.name <Nome Cognome>}: imposta il nome dell'utente, che comparirà come autore delle commit effettuate;
	  	\item \hicode{{git} config --global user.email <email>}: imposta l'email dell'utente; deve essere impostata la stessa email utilizzata per la registrazione su GitHub.
	  \end{itemize}
	  ...
	  \begin{itemize}
	  	...
	  	\item eseguire il comando: \hicode{git init};
	  	\item recuperare l'indirizzo URL del progetto su GitHub;
	  	\item eseguire il comando: \hicode{git clone <indirizzo appena recuperato>}.
	  \end{itemize}
	  ...
	  \begin{enumerate}
	  	\item eseguire \hicode{git pull} per scaricare le modifiche da remoto;
	  	\item se sono presenti conflitti:
	  	\begin{enumerate}
	  		\item eseguire \hicode{git stash} per salvare momentaneamente le modifiche locali;
	  		\item eseguire \hicode{git pull};
	  		\item eseguire \hicode{git stash apply} per ripristinare le modifiche;
	  	\end{enumerate}
	  	\item eseguire \hicode{git add NomeFile} per ognuno dei file in cui sono state effettuate delle modifiche;
	  	\item eseguire \hicode{git commit -m "descrizione modifiche"};
	  	\item eseguire \hicode{git push} per inviare le modifiche al repository remoto.
	  \end{enumerate}
	  \begin{figure}[H]
	  	\centering
        ...
        \subsubsection{Procedure}
	        ...
	        \begin{enumerate}
	        	...
	        	\item inserire nel codice sorgente uno dei seguenti commenti:
		        	\begin{itemize}
		        		\item \hicode{TODO} per segnalare un'aggiunta da eseguire al codice;
		        		\item \hicode{FIXME} per segnalare un errore rilevato per cui è stata proposta una soluzione;
		        		\item \hicode{BUG} per segnalare un'anomalia da rivedere e correggere;
		        	\end{itemize} 
	        ...
	        \begin{enumerate}
	        	...
	        	\item posizionarsi nella cartella \hicode{/script} all'interno dello spazio di lavoro condiviso \glo{Dropbox}{Dropbox};
	        	\item eseguire il comando: \hicode{gulpeasepdf.pl NomeDocumento.pdf};
	        ...
	        \begin{enumerate}
	        	\item aprire una riga di comando o terminale;
	        	\item posizionarsi nella cartella \hicode{/script} all'interno del repository desiderato (Documenti o DeGeOp);
	        	\item eseguire il comando \hicode{git pull} per assicurarsi che il repository sia aggiornato;
	        	\item eseguire il comando: \hicode{./checkLOCgit.sh [-d numero\_giorni]};
	        ...
	        \begin{enumerate}
	        	...
	        	\item posizionarsi nella cartella \hicode{/script} all'interno del repository desiderato (Documenti o DeGeOp);
	        	\item eseguire il comando \hicode{git pull} per assicurarsi che il repository sia aggiornato;
	        	\item eseguire il comando: \hicode{./checkRes.sh <dimensione\_minima>};
	        ...
	        \begin{enumerate}
	        	...
	        	\item posizionarsi nella cartella \hicode{/script} all'interno del repository desiderato (Documenti o DeGeOp);
	        	\item eseguire il comando \hicode{git pull} per assicurarsi che il repository sia aggiornato;
	        	\item eseguire il comando: \hicode{./checkSubSub.sh};
