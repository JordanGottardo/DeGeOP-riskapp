 ...
 * File: source/components/mapComponent/interactions/addScenarioInteraction.ts
...
/** parametri da passare a AddScenarioInteraction */
interface AddScenarioInteractionOptions {
...
/** interazione per aggiungere uno scenario */
class AddScenarioInteraction extends ol.interaction.Draw {
  ...
  constructor(options: AddScenarioInteractionOptions) {
    super({
      type: 'Polygon',
  ...
  private onAddFeature(ev: ol.Collection.Event) {
    console.log('AddScenarioInteraction once');
    if (this.features.getLength() >= 1) {
      // const feat = this.drawingFeat;
      const geo: any = this.drawingFeat.getGeometry();
      ...
      this.handleChange('application_area', geoshape);  // salvo l'ultimo poligono
    }
  }

}

export { AddScenarioInteraction };
